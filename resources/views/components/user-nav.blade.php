    ...
    <div class='flex flex-wrap items-center justify-between gap-5 relative'>
      ...
      <div class='flex lg:order-1 max-sm:ml-auto'>
        <form method="POST" action="{{ route('logout') }}">
            @csrf
        <button
          class='px-4 py-2 text-sm rounded-full font-bold text-white border-2 border-[#007bff] bg-[#007bff] transition-all ease-in-out duration-300 hover:bg-transparent hover:text-[#007bff]'>Log Out</button>
        <button
