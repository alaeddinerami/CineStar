<x-app-layout>
    ...
    <div class="w-11/12 h-[90vh] mx-auto flex flex-col items-start justify-start mt-8 mb-20 gap-8 text-gray-900">
        ...
        <p class="text-4xl font-semibold">My Reservations:</p>
        <div
            class="w-full flex flex-col gap-6 border-t-2 py-8 rounded-md shadow-lg p-3 h-[70vh] sm:h-[80vh] overflow-x-auto">
            @foreach ($reservations as $reservation)
                @php
                    foreach ($reservation->seat->hall->films as $f) {
                        if ($f->pivot->date == $reservation->screening_date) {
                            $film = $f;
                            break;
                        }
                    }
                @endphp
                <x-reservation-card :film="$film" :reservation="$reservation" />
            @endforeach

            <div class="mt-6 py-4">{{ $reservations->links() }}</div>
        </div>
    </div>
    @stack('vite')
